...
템플릿에서 제공되는 파라미터를 수정할 수 있다. 예를 들어, 가옹셩을 향상시키기 위해 다른 zone에서 VM을 실행하려면, multiple node pool을 지정할 수 있다. Node pools은 Kubernetes 클러스터에서 인스턴스 그룹이다. 관리형 인스턴스 그룹과 매우 유사하지만, 같지 않다.
